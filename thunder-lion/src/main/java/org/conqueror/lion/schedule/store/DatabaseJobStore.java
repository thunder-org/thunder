...
import static org.conqueror.lion.schedule.store.JobStoreSerializer.readSerializableObject;
import static org.conqueror.lion.schedule.store.JobStoreSerializer.writeSerializableObject;
...
public class DatabaseJobStore implements JobStore {
    ...
    static class JobKeySerializer implements JobStoreSerializer<JobKey> {

        @Override
        public void serialize(@NotNull DataOutput2 out, @NotNull JobKey value) throws IOException {
            out.writeUTF(value.getGroup());
            out.writeUTF(value.getName());
        }

        @Override
        public JobKey deserialize(@NotNull DataInput2 input, int available) throws IOException {
            return new JobKey(input.readUTF(), input.readUTF());
        }

    }

    static class TriggerKeySerializer implements JobStoreSerializer<TriggerKey> {

        @Override
        public void serialize(@NotNull DataOutput2 out, @NotNull TriggerKey value) throws IOException {
            out.writeUTF(value.getGroup());
            out.writeUTF(value.getName());
        }

        @Override
        public TriggerKey deserialize(@NotNull DataInput2 input, int available) throws IOException {
            return new TriggerKey(input.readUTF(), input.readUTF());
        }

    }

    static class JobWrapperSerializer implements JobStoreSerializer<JobWrapper> {

        @Override
        public void serialize(@NotNull DataOutput2 out, @NotNull JobWrapper value) throws IOException {
            JobDetail jobDetail = value.jobDetail;
            out.writeUTF(jobDetail.getKey().getGroup());
            out.writeUTF(jobDetail.getKey().getName());
            out.writeUTF(jobDetail.getDescription());
            out.writeUTF(jobDetail.getJobClass().getName());
            writeSerializableObject(out, jobDetail.getJobDataMap());
            out.writeBoolean(jobDetail.isDurable());
            out.writeBoolean(jobDetail.requestsRecovery());
        }

        @Override
        public JobWrapper deserialize(@NotNull DataInput2 input, int available) throws IOException {
            ...
            try {
                ...
                jobClass = (Class<? extends Job>) Class.forName(input.readUTF());
            } catch (ClassNotFoundException e) {
                throw new IOException(e.getMessage(), e.getCause());
            }
            JobDataMap dataMap = readSerializableObject(input);
            boolean durable = input.readBoolean();
            boolean recovery = input.readBoolean();
    ...
    static class TriggerWrapperSerializer implements JobStoreSerializer<TriggerWrapper> {

        @Override
        public void serialize(@NotNull DataOutput2 out, @NotNull TriggerWrapper value) throws IOException {
            writeSerializableObject(out, value.trigger);
        }

        @Override
        public TriggerWrapper deserialize(@NotNull DataInput2 input, int available) throws IOException {
            OperableTrigger trigger = readSerializableObject(input);
            return new TriggerWrapper(trigger);
        }

    }

    static class CalendarSerializer implements JobStoreSerializer<java.util.Calendar> {

        @Override
        public void serialize(@NotNull DataOutput2 out, @NotNull java.util.Calendar value) throws IOException {
            writeSerializableObject(out, value);
        }

        @Override
        public java.util.Calendar deserialize(@NotNull DataInput2 input, int available) throws IOException {
            return readSerializableObject(input);
        }
    ...
    private HashMap<JobKey, JobWrapper> jobsByKey = new HashMap<>(1000);

    private HashMap<TriggerKey, TriggerWrapper> triggersByKey = new HashMap<>(1000);

    private HashMap<String, HashMap<JobKey, JobWrapper>> jobsByGroup = new HashMap<>(25);

    private HashMap<String, HashMap<TriggerKey, TriggerWrapper>> triggersByGroup = new HashMap<>(25);

    private TreeSet<TriggerWrapper> timeTriggers = new TreeSet<>(new TriggerWrapperComparator());

    private HashMap<String, Calendar> calendarsByName = new HashMap<>(25);

    private ArrayList<TriggerWrapper> triggers = new ArrayList<>(1000);

    private HashSet<String> pausedTriggerGroups = new HashSet<>();

    private HashSet<String> pausedJobGroups = new HashSet<>();

    private HashSet<JobKey> blockedJobs = new HashSet<>();
    ...
    /**
     * <p>
     * Create a new <code>DatabaseJobStore</code>.
     * </p>
     */
    public DatabaseJobStore(String dbFilePath, long startDBSize) {
        try {
    ...
    private ArrayList<TriggerWrapper> getTriggerWrappersForCalendar(String calName) {
        ArrayList<TriggerWrapper> trigList = new ArrayList<>();
...
class TriggerWrapperComparator implements Comparator<TriggerWrapper>, java.io.Serializable {
    ...
    private static TriggerTimeComparator ttc = new TriggerTimeComparator();
